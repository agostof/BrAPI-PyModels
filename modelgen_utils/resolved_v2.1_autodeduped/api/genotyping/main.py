# generated by fastapi-codegen:
#   filename:  PlantBreedingAPI-BrAPI-Genotyping-2.1-resolved.yaml
#   timestamp: 2023-03-20T19:02:17+00:00

from __future__ import annotations

from typing import List, Optional, Union

from fastapi import FastAPI, Header, Path, Query
from pydantic import constr

from .models import (
    AlleleMatrixResponse,
    AlleleMatrixSearchRequest,
    Call,
    CallSetResponse,
    CallSetsListResponse,
    CallSetsSearchRequest,
    CallsListResponse,
    CallsSearchRequest,
    Field202AcceptedSearchResponse,
    GenomeMapListResponse,
    GenomeMapSingleResponse,
    LinkageGroupListResponse,
    MarkerPositionListResponse,
    MarkerPositionSearchRequest,
    PlateListResponse,
    PlateNewRequest,
    PlateSearchRequest,
    PlateSingleResponse,
    PlatesPutRequest,
    ReferenceBasesResponse,
    ReferenceSetsListResponse,
    ReferenceSetsSearchRequest,
    ReferenceSetsSingleResponse,
    ReferenceSingleResponse,
    ReferencesListResponse,
    ReferencesSearchRequest,
    SampleListResponse,
    SampleNewRequest,
    SampleSearchRequest,
    SampleSingleResponse,
    SamplesPutRequest,
    VariantSetResponse,
    VariantSetsExtractRequest,
    VariantSetsListResponse,
    VariantSetsSearchRequest,
    VariantSingleResponse,
    VariantsListResponse,
    VariantsSearchRequest,
    VendorOrderListResponse,
    VendorOrderStatusResponse,
    VendorOrderSubmissionRequest,
    VendorOrderSubmissionSingleResponse,
    VendorPlateListResponse,
    VendorPlateSubmissionIdSingleResponse,
    VendorPlateSubmissionRequest,
    VendorPlateSubmissionSingleResponse,
    VendorResultFileListResponse,
    VendorSpecificationSingleResponse,
)

app = FastAPI(
    title='BrAPI-Genotyping',
    description='The Breeding API (BrAPI) is a Standardized REST ful Web Service API Specification for communicating Plant Breeding Data. BrAPI allows for easy data sharing between databases and tools involved in plant breeding.\n<div class="brapi-section">\n<h2 class="brapi-section-title">General Reference Documentation</h2>\n<div class="gen-info-link"><a href="https://wiki.brapi.org/index.php/RESTfulness">URL Structure</a></div>\n<div class="gen-info-link"><a href="https://wiki.brapi.org/index.php/Response_Structure">Response Structure</a></div>\n<div class="gen-info-link"><a href="https://wiki.brapi.org/index.php/Dates_and_Times">Date/Time Encoding</a></div>\n<div class="gen-info-link"><a href="https://wiki.brapi.org/index.php/Location_Coordinates">Location Encoding</a></div>\n<div class="gen-info-link"><a href="https://wiki.brapi.org/index.php/Error_Handling">Error Handling</a></div>\n<div class="gen-info-link"><a href="https://wiki.brapi.org/index.php/Search_Services">Search Services</a></div>\n</div>\n\n<div class="brapi-section">\n<h2 class="brapi-section-title">BrAPI Core</h2>\n<div class="brapi-section-description">The BrAPI Core module contains high level entities used for organization and management. This includes Programs, Trials, Studies, Locations, People, and Lists</div>\n<div class="version-number">V2.1</div>\n<div class="link-btn"><a href="https://github.com/plantbreeding/API/tree/brapi-V2.1/Specification/BrAPI-Core">GitHub</a></div>\n<div class="link-btn"><a href="https://app.swaggerhub.com/apis/PlantBreedingAPI/BrAPI-Core/2.1">SwaggerHub</a></div>\n<div class="link-btn"><a href="https://brapicore21.docs.apiary.io">Apiary</a></div>\n<div class="stop-float"></div>\n</div>\n\n<div class="brapi-section">\n<h2 class="brapi-section-title">BrAPI Phenotyping</h2>\n<div class="brapi-section-description">The BrAPI Phenotyping module contains entities related to phenotypic observations. This includes Observation Units, Observations, Observation Variables, Traits, Scales, Methods, and Images</div>\n<div class="version-number">V2.1</div>\n<div class="link-btn"><a href="https://github.com/plantbreeding/API/tree/brapi-V2.1/Specification/BrAPI-Phenotyping">GitHub</a></div>\n<div class="link-btn"><a href="https://app.swaggerhub.com/apis/PlantBreedingAPI/BrAPI-Phenotyping/2.1">SwaggerHub</a></div>\n<div class="link-btn"><a href="https://brapiphenotyping21.docs.apiary.io">Apiary</a></div>\n<div class="stop-float"></div>\n</div>\n\n<div class="current-brapi-section brapi-section">\n<h2 class="brapi-section-title">BrAPI Genotyping</h2>\n<div class="brapi-section-description">The BrAPI Genotyping module contains entities related to genotyping analysis. This includes Samples, Markers, Variant Sets, Variants, Call Sets, Calls, References, Reads, and Vendor Orders</div>\n<div class="version-number">V2.1</div>\n<div class="link-btn"><a href="https://github.com/plantbreeding/API/tree/brapi-V2.1/Specification/BrAPI-Genotyping">GitHub</a></div>\n<div class="link-btn"><a href="https://app.swaggerhub.com/apis/PlantBreedingAPI/BrAPI-Genotyping/2.1">SwaggerHub</a></div>\n<div class="link-btn"><a href="https://brapigenotyping21.docs.apiary.io">Apiary</a></div>\n<div class="stop-float"></div>\n</div>\n\n<div class="brapi-section">\n<h2 class="brapi-section-title">BrAPI Germplasm</h2>\n<div class="brapi-section-description">The BrAPI Germplasm module contains entities related to germplasm management. This includes Germplasm, Germplasm Attributes, Seed Lots, Crosses, Pedigree, and Progeny</div>\n<div class="version-number">V2.1</div>\n<div class="link-btn"><a href="https://github.com/plantbreeding/API/tree/brapi-V2.1/Specification/BrAPI-Germplasm">GitHub</a></div>\n<div class="link-btn"><a href="https://app.swaggerhub.com/apis/PlantBreedingAPI/BrAPI-Germplasm/2.1">SwaggerHub</a></div>\n<div class="link-btn"><a href="https://brapigermplasm21.docs.apiary.io">Apiary</a></div>\n<div class="stop-float"></div>\n</div>\n\n<style>\n.link-btn{\nfloat: left; \nmargin: 2px 10px 0 0; \npadding: 0 5px; \nborder-radius: 5px; \nbackground-color: #ddd;\n}\n.stop-float{\n  clear: both;\n}\n.version-number{\n  float: left; \n  margin: 5px 10px 0 5px;\n}\n.brapi-section-title{\n  margin: 0 10px 0 0;\n  font-size: 20px;\n}\n.current-brapi-section{\n  font-weight: bolder;\n  border-radius: 5px; \n  background-color: #ddd;\n}\n.brapi-section{\n  padding: 5px 5px; \n}\n.brapi-section-description{\n  margin: 5px 0 0 5px;\n}\n</style>',
    version='2.1',
    servers=[{'url': 'https://test-server.brapi.org/brapi/v2'}],
)


@app.get(
    '/allelematrix',
    response_model=AlleleMatrixResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Allele Matrix'],
)
def get_allelematrix(
    dimension_variant_page: Optional[int] = Query(None, alias='dimensionVariantPage'),
    dimension_variant_page_size: Optional[int] = Query(
        None, alias='dimensionVariantPageSize'
    ),
    dimension_call_set_page: Optional[int] = Query(None, alias='dimensionCallSetPage'),
    dimension_call_set_page_size: Optional[int] = Query(
        None, alias='dimensionCallSetPageSize'
    ),
    preview: Optional[bool] = None,
    data_matrix_names: Optional[str] = Query(None, alias='dataMatrixNames'),
    data_matrix_abbreviations: Optional[str] = Query(
        None, alias='dataMatrixAbbreviations'
    ),
    position_range: Optional[str] = Query(None, alias='positionRange'),
    germplasm_db_id: Optional[str] = Query(None, alias='germplasmDbId'),
    germplasm_name: Optional[str] = Query(None, alias='germplasmName'),
    germplasm_p_u_i: Optional[str] = Query(None, alias='germplasmPUI'),
    call_set_db_id: Optional[str] = Query(None, alias='callSetDbId'),
    variant_db_id: Optional[str] = Query(None, alias='variantDbId'),
    variant_set_db_id: Optional[str] = Query(None, alias='variantSetDbId'),
    expand_homozygotes: Optional[bool] = Query(None, alias='expandHomozygotes'),
    unknown_string: Optional[str] = Query(None, alias='unknownString'),
    sep_phased: Optional[str] = Query(None, alias='sepPhased'),
    sep_unphased: Optional[str] = Query(None, alias='sepUnphased'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[AlleleMatrixResponse, str]:
    """
    Use this endpoint to retrieve a two dimensional matrix of genotype data
    """
    pass


@app.get(
    '/calls',
    response_model=CallsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Calls'],
)
def get_calls(
    call_set_db_id: Optional[str] = Query(None, alias='callSetDbId'),
    variant_db_id: Optional[str] = Query(None, alias='variantDbId'),
    variant_set_db_id: Optional[str] = Query(None, alias='variantSetDbId'),
    expand_homozygotes: Optional[bool] = Query(None, alias='expandHomozygotes'),
    unknown_string: Optional[str] = Query(None, alias='unknownString'),
    sep_phased: Optional[str] = Query(None, alias='sepPhased'),
    sep_unphased: Optional[str] = Query(None, alias='sepUnphased'),
    page_token: Optional[str] = Query(None, alias='pageToken'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[CallsListResponse, str]:
    """
    Gets a filtered list of `Calls`
    """
    pass


@app.put(
    '/calls',
    response_model=CallsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Calls'],
)
def put_calls(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: List[Call] = None,
) -> Union[CallsListResponse, str]:
    """
    Update existing `Calls` with new genotype value or metadata
    """
    pass


@app.get(
    '/callsets',
    response_model=CallSetsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Call Sets'],
)
def get_callsets(
    call_set_db_id: Optional[str] = Query(None, alias='callSetDbId'),
    call_set_name: Optional[str] = Query(None, alias='callSetName'),
    variant_set_db_id: Optional[str] = Query(None, alias='variantSetDbId'),
    sample_db_id: Optional[str] = Query(None, alias='sampleDbId'),
    germplasm_db_id: Optional[str] = Query(None, alias='germplasmDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    external_reference_id: Optional[str] = Query(None, alias='externalReferenceId'),
    external_reference_source: Optional[str] = Query(
        None, alias='externalReferenceSource'
    ),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[CallSetsListResponse, str]:
    """
    Gets a filtered list of `CallSet` JSON objects.
    """
    pass


@app.get(
    '/callsets/{call_set_db_id}',
    response_model=CallSetResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Call Sets'],
)
def get_callsets_call_set_db_id(
    call_set_db_id: str = Path(..., alias='callSetDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[CallSetResponse, str]:
    """
    Gets a `CallSet` by ID.
    """
    pass


@app.get(
    '/callsets/{call_set_db_id}/calls',
    response_model=CallsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Call Sets'],
)
def get_callsets_call_set_db_id_calls(
    call_set_db_id: str = Path(..., alias='callSetDbId'),
    expand_homozygotes: Optional[bool] = Query(None, alias='expandHomozygotes'),
    unknown_string: Optional[str] = Query(None, alias='unknownString'),
    sep_phased: Optional[str] = Query(None, alias='sepPhased'),
    sep_unphased: Optional[str] = Query(None, alias='sepUnphased'),
    page_token: Optional[str] = Query(None, alias='pageToken'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[CallsListResponse, str]:
    """
    Gets a list of `Calls` associated with a `CallSet`.
    """
    pass


@app.get(
    '/maps',
    response_model=GenomeMapListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Genome Maps'],
)
def get_maps(
    map_db_id: Optional[str] = Query(None, alias='mapDbId'),
    map_p_u_i: Optional[str] = Query(None, alias='mapPUI'),
    scientific_name: Optional[str] = Query(None, alias='scientificName'),
    type: Optional[str] = None,
    common_crop_name: Optional[str] = Query(None, alias='commonCropName'),
    program_db_id: Optional[str] = Query(None, alias='programDbId'),
    trial_db_id: Optional[str] = Query(None, alias='trialDbId'),
    study_db_id: Optional[str] = Query(None, alias='studyDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[GenomeMapListResponse, str]:
    """
    Get the Genomic Maps
    """
    pass


@app.get(
    '/maps/{map_db_id}',
    response_model=GenomeMapSingleResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Genome Maps'],
)
def get_maps_map_db_id(
    map_db_id: str = Path(..., alias='mapDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[GenomeMapSingleResponse, str]:
    """
    Get the details of a specific Genomic Map
    """
    pass


@app.get(
    '/maps/{map_db_id}/linkagegroups',
    response_model=LinkageGroupListResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Genome Maps'],
)
def get_maps_map_db_id_linkagegroups(
    map_db_id: str = Path(..., alias='mapDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[LinkageGroupListResponse, str]:
    """
    Get the Linkage Groups of a specific Genomic Map
    """
    pass


@app.get(
    '/markerpositions',
    response_model=MarkerPositionListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Genome Maps'],
)
def get_markerpositions(
    map_db_id: Optional[str] = Query(None, alias='mapDbId'),
    linkage_group_name: Optional[str] = Query(None, alias='linkageGroupName'),
    variant_db_id: Optional[str] = Query(None, alias='variantDbId'),
    min_position: Optional[int] = Query(None, alias='minPosition'),
    max_position: Optional[int] = Query(None, alias='maxPosition'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[MarkerPositionListResponse, str]:
    """
    Get marker position info
    """
    pass


@app.get(
    '/plates',
    response_model=PlateListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Plates'],
)
def get_plates(
    sample_db_id: Optional[str] = Query(None, alias='sampleDbId'),
    sample_name: Optional[str] = Query(None, alias='sampleName'),
    sample_group_db_id: Optional[str] = Query(None, alias='sampleGroupDbId'),
    observation_unit_db_id: Optional[str] = Query(None, alias='observationUnitDbId'),
    plate_db_id: Optional[str] = Query(None, alias='plateDbId'),
    plate_name: Optional[str] = Query(None, alias='plateName'),
    common_crop_name: Optional[str] = Query(None, alias='commonCropName'),
    program_db_id: Optional[str] = Query(None, alias='programDbId'),
    trial_db_id: Optional[str] = Query(None, alias='trialDbId'),
    study_db_id: Optional[str] = Query(None, alias='studyDbId'),
    germplasm_db_id: Optional[str] = Query(None, alias='germplasmDbId'),
    external_reference_i_d: Optional[str] = Query(None, alias='externalReferenceID'),
    external_reference_id: Optional[str] = Query(None, alias='externalReferenceId'),
    external_reference_source: Optional[str] = Query(
        None, alias='externalReferenceSource'
    ),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[PlateListResponse, str]:
    """
    Get a filtered list of Plates.
    """
    pass


@app.put(
    '/plates',
    response_model=PlateListResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Plates'],
)
def put_plates(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: PlatesPutRequest = None,
) -> Union[PlateListResponse, str]:
    """
    Update the details of existing Plates
    """
    pass


@app.post(
    '/plates',
    response_model=PlateListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Plates'],
)
def post_plates(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: List[PlateNewRequest] = None,
) -> Union[PlateListResponse, str]:
    """
    Submit new Plate entities to the server
    """
    pass


@app.get(
    '/plates/{plate_db_id}',
    response_model=PlateSingleResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Plates'],
)
def get_plates_plate_db_id(
    plate_db_id: str = Path(..., alias='plateDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[PlateSingleResponse, str]:
    """
    Get the details of a specific Plate.
    """
    pass


@app.get(
    '/references',
    response_model=ReferencesListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['References'],
)
def get_references(
    reference_db_id: Optional[str] = Query(None, alias='referenceDbId'),
    reference_set_db_id: Optional[str] = Query(None, alias='referenceSetDbId'),
    accession: Optional[str] = None,
    md5checksum: Optional[str] = None,
    is_derived: Optional[bool] = Query(None, alias='isDerived'),
    min_length: Optional[int] = Query(None, alias='minLength'),
    max_length: Optional[int] = Query(None, alias='maxLength'),
    common_crop_name: Optional[str] = Query(None, alias='commonCropName'),
    program_db_id: Optional[str] = Query(None, alias='programDbId'),
    trial_db_id: Optional[str] = Query(None, alias='trialDbId'),
    study_db_id: Optional[str] = Query(None, alias='studyDbId'),
    external_reference_id: Optional[str] = Query(None, alias='externalReferenceId'),
    external_reference_source: Optional[str] = Query(
        None, alias='externalReferenceSource'
    ),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[ReferencesListResponse, str]:
    """
    Gets a filtered list of `Reference` objects.
    """
    pass


@app.get(
    '/references/{reference_db_id}',
    response_model=ReferenceSingleResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['References'],
)
def get_references_reference_db_id(
    reference_db_id: str = Path(..., alias='referenceDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[ReferenceSingleResponse, str]:
    """
    Gets a `Reference` by ID.
    """
    pass


@app.get(
    '/references/{reference_db_id}/bases',
    response_model=ReferenceBasesResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['References'],
)
def get_references_reference_db_id_bases(
    reference_db_id: str = Path(..., alias='referenceDbId'),
    start: Optional[int] = None,
    end: Optional[int] = None,
    page_token: Optional[str] = Query(None, alias='pageToken'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[ReferenceBasesResponse, str]:
    """
    Lists `Reference` bases by ID and optional range.
    """
    pass


@app.get(
    '/referencesets',
    response_model=ReferenceSetsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Reference Sets'],
)
def get_referencesets(
    reference_set_db_id: Optional[str] = Query(None, alias='referenceSetDbId'),
    accession: Optional[str] = None,
    assembly_p_u_i: Optional[str] = Query(None, alias='assemblyPUI'),
    md5checksum: Optional[str] = None,
    common_crop_name: Optional[str] = Query(None, alias='commonCropName'),
    program_db_id: Optional[str] = Query(None, alias='programDbId'),
    trial_db_id: Optional[str] = Query(None, alias='trialDbId'),
    study_db_id: Optional[str] = Query(None, alias='studyDbId'),
    external_reference_id: Optional[str] = Query(None, alias='externalReferenceId'),
    external_reference_source: Optional[str] = Query(
        None, alias='externalReferenceSource'
    ),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[ReferenceSetsListResponse, str]:
    """
    Gets a list of `ReferenceSets`.
    """
    pass


@app.get(
    '/referencesets/{reference_set_db_id}',
    response_model=ReferenceSetsSingleResponse,
    tags=['Reference Sets'],
)
def get_referencesets_reference_set_db_id(
    reference_set_db_id: str = Path(..., alias='referenceSetDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> ReferenceSetsSingleResponse:
    """
    Gets a `ReferenceSet` by ID.
    """
    pass


@app.get(
    '/samples',
    response_model=SampleListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Samples'],
)
def get_samples(
    sample_db_id: Optional[str] = Query(None, alias='sampleDbId'),
    sample_name: Optional[str] = Query(None, alias='sampleName'),
    sample_group_db_id: Optional[str] = Query(None, alias='sampleGroupDbId'),
    observation_unit_db_id: Optional[str] = Query(None, alias='observationUnitDbId'),
    plate_db_id: Optional[str] = Query(None, alias='plateDbId'),
    plate_name: Optional[str] = Query(None, alias='plateName'),
    common_crop_name: Optional[str] = Query(None, alias='commonCropName'),
    program_db_id: Optional[str] = Query(None, alias='programDbId'),
    trial_db_id: Optional[str] = Query(None, alias='trialDbId'),
    study_db_id: Optional[str] = Query(None, alias='studyDbId'),
    germplasm_db_id: Optional[str] = Query(None, alias='germplasmDbId'),
    external_reference_i_d: Optional[str] = Query(None, alias='externalReferenceID'),
    external_reference_id: Optional[str] = Query(None, alias='externalReferenceId'),
    external_reference_source: Optional[str] = Query(
        None, alias='externalReferenceSource'
    ),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[SampleListResponse, str]:
    """
    Get the Samples
    """
    pass


@app.put(
    '/samples',
    response_model=SampleListResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Samples'],
)
def put_samples(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: SamplesPutRequest = None,
) -> Union[SampleListResponse, str]:
    """
    Update the details of existing Samples
    """
    pass


@app.post(
    '/samples',
    response_model=SampleListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Samples'],
)
def post_samples(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: List[SampleNewRequest] = None,
) -> Union[SampleListResponse, str]:
    """
    Add new Samples
    """
    pass


@app.get(
    '/samples/{sample_db_id}',
    response_model=SampleSingleResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Samples'],
)
def get_samples_sample_db_id(
    sample_db_id: str = Path(..., alias='sampleDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[SampleSingleResponse, str]:
    """
    Get the details of a specific Sample
    """
    pass


@app.put(
    '/samples/{sample_db_id}',
    response_model=SampleSingleResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Samples'],
)
def put_samples_sample_db_id(
    sample_db_id: str = Path(..., alias='sampleDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: SampleNewRequest = None,
) -> Union[SampleSingleResponse, str]:
    """
    Update the details of an existing Sample
    """
    pass


@app.post(
    '/search/allelematrix',
    response_model=AlleleMatrixResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Allele Matrix'],
)
def post_search_allelematrix(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: AlleleMatrixSearchRequest = None,
) -> Union[AlleleMatrixResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for a Allele Matrix
    """
    pass


@app.get(
    '/search/allelematrix/{search_results_db_id}',
    response_model=AlleleMatrixResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Allele Matrix'],
)
def get_search_allelematrix_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[AlleleMatrixResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a Allele Matrix search request
    """
    pass


@app.post(
    '/search/calls',
    response_model=CallsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Calls'],
)
def post_search_calls(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: CallsSearchRequest = None,
) -> Union[CallsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for `Calls`
    """
    pass


@app.get(
    '/search/calls/{search_results_db_id}',
    response_model=CallsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Calls'],
)
def get_search_calls_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    page_token: Optional[str] = Query(None, alias='pageToken'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[CallsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a `Calls` search request
    """
    pass


@app.post(
    '/search/callsets',
    response_model=CallSetsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Call Sets'],
)
def post_search_callsets(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: CallSetsSearchRequest = None,
) -> Union[CallSetsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for `CallSets`
    """
    pass


@app.get(
    '/search/callsets/{search_results_db_id}',
    response_model=CallSetsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Call Sets'],
)
def get_search_callsets_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[CallSetsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a `CallSets` search request
    """
    pass


@app.post(
    '/search/markerpositions',
    response_model=MarkerPositionListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Genome Maps'],
)
def post_search_markerpositions(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: MarkerPositionSearchRequest = None,
) -> Union[MarkerPositionListResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for `MarkerPositions`
    """
    pass


@app.get(
    '/search/markerpositions/{search_results_db_id}',
    response_model=MarkerPositionListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Genome Maps'],
)
def get_search_markerpositions_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[MarkerPositionListResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a `MarkerPositions` search request
    """
    pass


@app.post(
    '/search/plates',
    response_model=PlateListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Plates'],
)
def post_search_plates(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: PlateSearchRequest = None,
) -> Union[PlateListResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for `Plates`
    """
    pass


@app.get(
    '/search/plates/{search_results_db_id}',
    response_model=PlateListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Plates'],
)
def get_search_plates_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[PlateListResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a `Plates` search request
    """
    pass


@app.post(
    '/search/references',
    response_model=ReferencesListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['References'],
)
def post_search_references(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: ReferencesSearchRequest = None,
) -> Union[ReferencesListResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for `References`
    """
    pass


@app.get(
    '/search/references/{search_results_db_id}',
    response_model=ReferencesListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['References'],
)
def get_search_references_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[ReferencesListResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a `References` search request
    """
    pass


@app.post(
    '/search/referencesets',
    response_model=ReferenceSetsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Reference Sets'],
)
def post_search_referencesets(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: ReferenceSetsSearchRequest,
) -> Union[ReferenceSetsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for `ReferenceSets`
    """
    pass


@app.get(
    '/search/referencesets/{search_results_db_id}',
    response_model=ReferenceSetsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Reference Sets'],
)
def get_search_referencesets_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[ReferenceSetsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a `ReferenceSets` search request
    """
    pass


@app.post(
    '/search/samples',
    response_model=SampleListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Samples'],
)
def post_search_samples(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: SampleSearchRequest = None,
) -> Union[SampleListResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for `Samples`
    """
    pass


@app.get(
    '/search/samples/{search_results_db_id}',
    response_model=SampleListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Samples'],
)
def get_search_samples_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[SampleListResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a `Samples` search request
    """
    pass


@app.post(
    '/search/variants',
    response_model=VariantsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Variants'],
)
def post_search_variants(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: VariantsSearchRequest = None,
) -> Union[VariantsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for `Variants`
    """
    pass


@app.get(
    '/search/variants/{search_results_db_id}',
    response_model=VariantsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Variants'],
)
def get_search_variants_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    page_token: Optional[str] = Query(None, alias='pageToken'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VariantsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a `Variants` search request
    """
    pass


@app.post(
    '/search/variantsets',
    response_model=VariantSetsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Variant Sets'],
)
def post_search_variantsets(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: VariantSetsSearchRequest = None,
) -> Union[VariantSetsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Submit a search request for `VariantSets`
    """
    pass


@app.get(
    '/search/variantsets/{search_results_db_id}',
    response_model=VariantSetsListResponse,
    responses={
        '202': {'model': Field202AcceptedSearchResponse},
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
    },
    tags=['Variant Sets'],
)
def get_search_variantsets_search_results_db_id(
    search_results_db_id: str = Path(..., alias='searchResultsDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VariantSetsListResponse, Field202AcceptedSearchResponse, str]:
    """
    Get the results of a `VariantSets` search request
    """
    pass


@app.get(
    '/variants',
    response_model=VariantsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Variants'],
)
def get_variants(
    variant_db_id: Optional[str] = Query(None, alias='variantDbId'),
    variant_set_db_id: Optional[str] = Query(None, alias='variantSetDbId'),
    reference_db_id: Optional[str] = Query(None, alias='referenceDbId'),
    reference_set_db_id: Optional[str] = Query(None, alias='referenceSetDbId'),
    page_token: Optional[str] = Query(None, alias='pageToken'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    external_reference_id: Optional[str] = Query(None, alias='externalReferenceId'),
    external_reference_source: Optional[str] = Query(
        None, alias='externalReferenceSource'
    ),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VariantsListResponse, str]:
    """
    Gets a filtered list of `Variants`.
    """
    pass


@app.get(
    '/variants/{variant_db_id}',
    response_model=VariantSingleResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Variants'],
)
def get_variants_variant_db_id(
    variant_db_id: str = Path(..., alias='variantDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VariantSingleResponse, str]:
    """
    Gets a `Variant` by ID.
    """
    pass


@app.get(
    '/variants/{variant_db_id}/calls',
    response_model=CallsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Variants'],
)
def get_variants_variant_db_id_calls(
    variant_db_id: str = Path(..., alias='variantDbId'),
    expand_homozygotes: Optional[bool] = Query(None, alias='expandHomozygotes'),
    unknown_string: Optional[str] = Query(None, alias='unknownString'),
    sep_phased: Optional[str] = Query(None, alias='sepPhased'),
    sep_unphased: Optional[str] = Query(None, alias='sepUnphased'),
    page_token: Optional[str] = Query(None, alias='pageToken'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[CallsListResponse, str]:
    """
    Gets a list of `Calls` associated with a `Variant`.
    """
    pass


@app.get(
    '/variantsets',
    response_model=VariantSetsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Variant Sets'],
)
def get_variantsets(
    variant_set_db_id: Optional[str] = Query(None, alias='variantSetDbId'),
    variant_db_id: Optional[str] = Query(None, alias='variantDbId'),
    call_set_db_id: Optional[str] = Query(None, alias='callSetDbId'),
    reference_set_db_id: Optional[str] = Query(None, alias='referenceSetDbId'),
    common_crop_name: Optional[str] = Query(None, alias='commonCropName'),
    program_db_id: Optional[str] = Query(None, alias='programDbId'),
    study_db_id: Optional[str] = Query(None, alias='studyDbId'),
    study_name: Optional[str] = Query(None, alias='studyName'),
    external_reference_id: Optional[str] = Query(None, alias='externalReferenceId'),
    external_reference_source: Optional[str] = Query(
        None, alias='externalReferenceSource'
    ),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VariantSetsListResponse, str]:
    """
    Gets a filtered list of `VariantSets`.
    """
    pass


@app.post(
    '/variantsets/extract',
    response_model=VariantSetResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Variant Sets'],
)
def post_variantsets_extract(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: VariantSetsExtractRequest = None,
) -> Union[VariantSetResponse, str]:
    """
    Create new `VariantSet` based on search results
    """
    pass


@app.get(
    '/variantsets/{variant_set_db_id}',
    response_model=VariantSetResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Variant Sets'],
)
def get_variantsets_variant_set_db_id(
    variant_set_db_id: str = Path(..., alias='variantSetDbId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VariantSetResponse, str]:
    """
    Gets a `VariantSet` by ID.
    """
    pass


@app.get(
    '/variantsets/{variant_set_db_id}/calls',
    response_model=CallsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Variant Sets'],
)
def get_variantsets_variant_set_db_id_calls(
    variant_set_db_id: str = Path(..., alias='variantSetDbId'),
    expand_homozygotes: Optional[bool] = Query(None, alias='expandHomozygotes'),
    unknown_string: Optional[str] = Query(None, alias='unknownString'),
    sep_phased: Optional[str] = Query(None, alias='sepPhased'),
    sep_unphased: Optional[str] = Query(None, alias='sepUnphased'),
    page_token: Optional[str] = Query(None, alias='pageToken'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[CallsListResponse, str]:
    """
    Gets a list of `Calls` associated with a `VariantSet`.
    """
    pass


@app.get(
    '/variantsets/{variant_set_db_id}/callsets',
    response_model=CallSetsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Variant Sets'],
)
def get_variantsets_variant_set_db_id_callsets(
    call_set_db_id: Optional[str] = Query(None, alias='callSetDbId'),
    call_set_name: Optional[str] = Query(None, alias='callSetName'),
    variant_set_db_id: str = Path(..., alias='variantSetDbId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[CallSetsListResponse, str]:
    """
    Gets a list of `CallSets` associated with a `VariantSet`.
    """
    pass


@app.get(
    '/variantsets/{variant_set_db_id}/variants',
    response_model=VariantsListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Variant Sets'],
)
def get_variantsets_variant_set_db_id_variants(
    variant_db_id: Optional[str] = Query(None, alias='variantDbId'),
    variant_set_db_id: str = Path(..., alias='variantSetDbId'),
    page_token: Optional[str] = Query(None, alias='pageToken'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VariantsListResponse, str]:
    """
    Gets a `Variants` for a given `VariantSet`.
    """
    pass


@app.get(
    '/vendor/orders',
    response_model=VendorOrderListResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Vendor'],
)
def get_vendor_orders(
    order_id: Optional[str] = Query(None, alias='orderId'),
    submission_id: Optional[str] = Query(None, alias='submissionId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VendorOrderListResponse, str]:
    """
    List current available orders
    """
    pass


@app.post(
    '/vendor/orders',
    response_model=VendorOrderSubmissionSingleResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Vendor'],
)
def post_vendor_orders(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: VendorOrderSubmissionRequest = None,
) -> Union[VendorOrderSubmissionSingleResponse, str]:
    """
    Submit New Order
    """
    pass


@app.get(
    '/vendor/orders/{order_id}/plates',
    response_model=VendorPlateListResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Vendor'],
)
def get_vendor_orders_order_id_plates(
    order_id: str = Path(..., alias='orderId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VendorPlateListResponse, str]:
    """
    Get the Plates for a specific Order
    """
    pass


@app.get(
    '/vendor/orders/{order_id}/results',
    response_model=VendorResultFileListResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Vendor'],
)
def get_vendor_orders_order_id_results(
    order_id: str = Path(..., alias='orderId'),
    page: Optional[int] = None,
    page_size: Optional[int] = Query(None, alias='pageSize'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VendorResultFileListResponse, str]:
    """
    Get the results of a specific Order
    """
    pass


@app.get(
    '/vendor/orders/{order_id}/status',
    response_model=VendorOrderStatusResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Vendor'],
)
def get_vendor_orders_order_id_status(
    order_id: str = Path(..., alias='orderId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VendorOrderStatusResponse, str]:
    """
    Get the status of a specific Order
    """
    pass


@app.post(
    '/vendor/plates',
    response_model=VendorPlateSubmissionIdSingleResponse,
    responses={'400': {'model': str}, '401': {'model': str}, '403': {'model': str}},
    tags=['Vendor'],
)
def post_vendor_plates(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
    body: VendorPlateSubmissionRequest = None,
) -> Union[VendorPlateSubmissionIdSingleResponse, str]:
    """
    Submit a new set of Sample data
    """
    pass


@app.get(
    '/vendor/plates/{submission_id}',
    response_model=VendorPlateSubmissionSingleResponse,
    responses={
        '400': {'model': str},
        '401': {'model': str},
        '403': {'model': str},
        '404': {'model': str},
    },
    tags=['Vendor'],
)
def get_vendor_plates_submission_id(
    submission_id: str = Path(..., alias='submissionId'),
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    ),
) -> Union[VendorPlateSubmissionSingleResponse, str]:
    """
    Get the data for a submitted set of plates
    """
    pass


@app.get(
    '/vendor/specifications',
    response_model=VendorSpecificationSingleResponse,
    tags=['Vendor'],
)
def get_vendor_specifications(
    authorization: Optional[constr(regex=r'^Bearer .*$')] = Header(
        None, alias='Authorization'
    )
) -> VendorSpecificationSingleResponse:
    """
    Get the Vendor Specifications
    """
    pass
